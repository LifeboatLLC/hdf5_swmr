...
configure_test_env() {
    case "$1" in
        standard)
            ...
            EXPECTED_H5_FILE_A="vfd_swmr_data_a.h5"
            EXPECTED_H5_FILE_B="vfd_swmr_data_b.h5"
        ...
        bigset)
            ...
            EXPECTED_H5_FILE_A="vfd_swmr_bigset_a.h5"
            EXPECTED_H5_FILE_B="vfd_swmr_bigset_b.h5"
...
run_test_case() {
    ...
    then
        ...
        vprintf "FAILED -- $EXPECTED_H5_FILE_B does not exist\n"
        nerrors=$((nerrors + 1))
        return 0
    fi
...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            -h)
                ...
                ;;
            -d)
                shift # move to next argument in args list
                ...
                # validate format: 0, 0.5, 1.1, 2.125, etc.
                if ! [[ "$FIXED_DELAY" =~ ^[0-9]+(\.[0-9]{1,3})?$ ]]; then
                    echo "ERROR: invalid time value '$FIXED_DELAY'"
                    echo "Expected format like 0.5 or 1.1 (up to 3 decimal places)"
                    exit 1
                fi
    ...
    # If fixed delay mode is enabled, only one test is allowed
    if $USE_FIXED_DELAY; then
        if [ ${#tests_to_run[@]} -gt 1 ]; then
            echo "ERROR: -d (fixed delay mode) can only be used with a single test"
            echo "Selected tests: ${tests_to_run[*]}"
            usage
            exit 1
        fi
    fi
