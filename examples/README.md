...
**Requirement:** When running over NFS, users must ensure that all program and file paths are valid on the system where each process is executed. In particular, the `--swmr-config-file` argument passed to `h5ls` must use an absolute path to the configuration file.
...
Before starting the auxiliary process, update the SWMR configuration file to set the metadata file location to a local non-NFS mounted directory. Only do this on the reader system:
```bash
...
> **Note:** When running over NFS, `aux_process` may detect and apply updater files in bursts rather than continuously because of NFS latency. While `aux_process` is waiting for additional updater files to become visible, the reader `rgaussians` may show blank output. This behavior is expected and does not indicate that either process has stalled.

**Terminal 1 (writer system):**  
Run the demo from the NFS-mounted directory:
```bash
HDF5_VFD_SWMR_CONFIG=/path/to/gaussians_swmr_gen_updater_config.txt /path/to/wgaussians
```

**Terminal 2 (reader system - setup):**  
Before starting the auxiliary process, update the SWMR configuration file to set the metadata file location to a local non-NFS mounted directory. Only do this on the reader system:
```bash
sed -i 's|( md_file_path "./" )|( md_file_path "/absolute/path/to/metadata_directory/" )|' gaussians_swmr_gen_updater_config.txt
```

**Terminal 2 (reader system - auxiliary process):**  
Start the auxiliary process after the writer has started:
```bash
/path/to/aux_process -v /path/to/metadata_directory/gaussians_md_file /path/to/NFS/mounted/directory/gaussians_updater_file
```

**Terminal 3 (reader system - reader process):**  
Start `rgaussians` after the auxiliary process has been started:
```bash
HDF5_VFD_SWMR_CONFIG=/path/to/gaussians_swmr_gen_updater_config.txt /path/to/rgaussians
```
## Legacy HDF5 Examples (DO NOT USE)

The following section is from the original HDF5 example distribution and is known to be outdated and incorrect.

> Running `run-all-ex.sh` will likely result in errors and is not supported.
